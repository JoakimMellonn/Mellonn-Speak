...
var s3 = new AWSS3();
const fileTypes = ['wav', 'flac', 'm4p', 'm4a', 'm4b', 'mmf', 'aac', 'mp3', 'mp4']; // Make this more better
const maxKeys = 1000;
...
async function checkList(list) {
    for (let file of list) {
        const filetype = file.Key.split('.')[1];
        if (filetype == null || filetype == undefined) continue;
        if (fileTypes.includes(filetype.toLowerCase())) {
            var expiryDate = new Date(file.LastModified);
            expiryDate.setDate(expiryDate.getDate() + expireTime);
